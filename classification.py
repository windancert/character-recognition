...
class Classification(ABC):
    ...
    def train(self, data: List[Tuple[str, Image]]):
        ...
        images_as_matrix = self.__create_matrix_of_images(images).T
        classifications_as_matrix = self.__create_matrix_of_classifications(classifications).T
        print(images_as_matrix.shape)
        print(images_as_matrix)
        print(classifications_as_matrix.shape)
        print(classifications_as_matrix)
    ...
    def __create_matrix_of_classifications(self, classifications: Tuple[str, ...]) -> ndarray:
        if self.__classification_to_index is None:
            raise Exception("Classification has not been trained yet")
        classification_matrix = numpy.zeros((len(classifications), len(self.__classification_to_index)))
        for i, classification in enumerate(classifications):
            classification_matrix[i, self.__classification_to_index[classification]] = 1
